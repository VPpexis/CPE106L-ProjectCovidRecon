...
## Actors
1. **Customer User** - The customer is the user who uses or benefit from the software application. This is an abstraction or generalization of the users as they all perform similar action but for different reasons. The actions that they may perform are: (1) to show or display the overview of the status of COVID-19 in Metro Manila, (2) visualize the data into charts for demonstrating the relationships, increments and decrements as well as the patterns in each data points through pie chart, bar chart and line charts, (3) show geographical location of the cases in Metro Manila by the whole region or a specific city, (4) and predict the number of cases/growth through logarithmic and exponential models.
...
* Customer Use Cases: Display (Overview)
    ...
    **_Brief Description_**
    ...
    This use case describes the main interface of the software application in a workflow. It shows the current status and the percentile of the total number of case, total number of deaths and total number of recovered patients limited only in the National Capital Region (NCR) in the Philippines. The cities covered in the statistical overview in this window are: Manila, Quezon City, Caloocan, Las Piñas, Makati, Malabon, Mandaluyong, Marikina, Muntinlupa, Navotas, Parañaque, Pasay, Pasig, San Juan, Taguig and Valenzuela. The data stored in the database reflects in this use case since all of the information regarding the latest number of COVID-19 patients are updated time to time through scraping.
    ...
    **_Typical Flow of Events:_**
    ...
    1. A window is generated and hies the main window.
...
* Customer Use Cases Illustrate Charts (Details)
    ...
    **_Brief Descriptions:_**
    ...
    This use case is an visual representation of the Display Status use case. It show case also the number of cases, recovered, and deaths of COVID – 19 cases thru a Pie chart, Bar chart, Line Chart. The charts are renewed time to time thru the web scraping of the developers to update the database.
...
The user interface for the system allow the user to get the latest updates regarding the COVID-19 cases in the NCR Region. The user can also access and visualize the data regarding the COVID-19 in the NCR region. It also shows a graph of the progress of the COVID-19 in the NCR. Lastly, it also shows the cases per each cities in the NCR. The interface has buttons according to the data it needs to visualize. This allows for ease of navigation using only the mouse.
